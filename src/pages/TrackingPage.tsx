...
export default function TrackingPage() {
  ...
  const STATUS_LABELS: Record<ShipmentStatus, string> = {
    picked_up: t.track_status_picked,
    in_transit: t.track_status_transit,
    customs: t.track_status_customs,
    out_for_delivery: t.track_status_delivery,
    delivered: t.track_status_delivered,
  };
  ...
  const search = (number?: string) => {
    ...
    if (s) { setShipment(s); setSimulatedStatus(s.status); setNotFound(false); }
    else { setShipment(null); setNotFound(true); }
  };

  useEffect(() => { if (searchParams.get('number')) search(searchParams.get('number')!); }, []);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setProgress(prev => { if (prev >= targetProgress) { clearInterval(timer); return targetProgress; } return prev + 1; });
    }, 30);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (idx < STATUS_ORDER.length - 1) setSimulatedStatus(STATUS_ORDER[idx + 1]);
    }, 5000);
    ...
    <div className="min-h-screen py-8 md:py-16">
      <div className="container max-w-3xl">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-2">{t.track_title}</h1>
          <p className="text-muted-foreground">{t.track_subtitle}</p>
        </div>

        <form onSubmit={e => { e.preventDefault(); search(); }} className="flex gap-3 mb-8">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
            <Input value={trackingInput} onChange={e => setTrackingInput(e.target.value)} placeholder="EE1234567890" className="pl-10 h-12" aria-label="Tracking number" />
          </div>
          <Button type="submit" className="h-12 px-8 bg-eagle-orange hover:bg-eagle-orange-hover text-white">{t.track_btn}</Button>
        </form>
        ...
        {notFound && (
          <Card className="animate-fade-in">
            <CardContent className="text-center py-8">
              ...
              <p className="font-medium">{t.track_not_found}</p>
            </CardContent>
          </Card>
        )}
        ...
        {shipment && (
          <div className="space-y-6 animate-fade-in">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <span className={`text-sm px-3 py-1 rounded-full ${simulatedStatus === 'delivered' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400'}`}>
            ...
            <Card>
              <CardHeader><CardTitle className="text-lg">{t.track_live_updates}</CardTitle></CardHeader>
              <CardContent>
                  ...
                  {STATUS_ORDER.map((s, i) => {
                    ...
                    return (
                      <div key={s} className={`flex gap-3 ${reached ? '' : 'opacity-40'}`}>
                        <div className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 ${reached ? 'bg-eagle-orange text-white' : 'bg-secondary text-muted-foreground'}`}>
                          <Icon className="h-4 w-4" />
                        </div>
                        <div>
                          <p className="font-medium text-sm">{STATUS_LABELS[s]}</p>
                          <p className="text-xs text-muted-foreground">{reached ? getLocationFeedback(shipment.origin, shipment.destination, s) : 'Pending'}</p>
                        </div>
                      </div>
                    );
                  })}
